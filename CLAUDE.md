...
1. **`MD/System_Principles.md`** — the nine rules the system runs by. If you propose a change that violates one, name which and why.
2. **`JOURNEY.md`** — short build story, the arc.
3. **`MD/Feature Design - Repeating Entities.md`** — the multi-entry mechanism (SHIPPED).
4. **`MD/Feature Design - Derived Variables.md`** — server-as-authority for computed fields (SHIPPED).
5. **`MD/Feature Design - Story Track + Visual Vocabulary.md`** — the Story track and per-section colors (SHIPPED).
6. **`MD/Intake Card Authoring Kit.md`** — the user-facing intake-card grammar, regenerable.
...
A fifth rail probably gets added someday. The pattern is documented in `System_Principles.md` §9.
...
**Run the test suite** (93 assertions at time of writing — keep it green):
```powershell
cd Frontend
python test_api.py
```
...
```
Reads SSOT from `DB/build_synthesis_db.py`, writes `MD/Intake Card Authoring Kit.md`.
